...
<head>
<meta charset="utf-8">
<link rel="stylesheet" type="text/css" href="/news/src/main/webapp/WEB-INF/css/updateUser.css">
...
<!----------------------reg form-------------------------------------->
    <tr>
		...
		$con = mysqli_connect("localhost","root","","newsdb");
				...
				$sql ="SELECT * FROM `usertb` WHERE `email`='".$_SESSION["email"]."'";
      ...
      <td>
		   ...
		   <form action="updateUser.php?id=<?php echo $_SESSION["email"];?>" method="post">
  ...
  <tbody>	
    ...
    <tr>
      <td height="58" colspan="3"><h1 align="center">UPDATE MY DETAILS</h1></td>
      </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    ...
    <tr>
      <td>FULL NAME</td>
      <td><input type="text" name="txtFullName" id="txtFullName" pattern="[a-zA-Z]+" title="Full Name must have alphabet characters only !" value="<?php echo $row['fullName'];?>" required></td>
      <td>&nbsp;</td>
    </tr>
	<tr>
      <td>E-MAIL</td>
      <td><input type="text" name="txtEmail" id="txtEmail" placeholder="abc@example.com" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" title="email must be XXX@XXXXX.XXX pattern" value="<?php echo $row['email'];?>" required ></td>
      <td>&nbsp;</td>
    </tr>
	 <tr>
      <td>AGE</td>
      <td><input type="text" name="txtAge" id="txtAge" pattern="[0-9.]+" title="Age must have numeric characters only!" value="<?php echo $row['age'];?>" required></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>ADDRESS</td>
      <td><input type="text" name="txtAddress" id="txtAddress" value="<?php echo $row['address'];?>" required></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>GENDER</td>
      <td><input type="text" name="txtGender" id="txtGender" value="<?php echo $row['gender'];?>" required></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>JOB</td>
      <td><input type="text" name="txtJob" id="txtjob" value="<?php echo $row['job'];?>" required></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>PASSWORD</td>
    ...
    <tr>
      ...
      <td><input type="password" name="txtConfirmPassword" id="txtConfirmPassword" required/></td>
			  			  ...
			  			  if(isset($_POST['btnSubmit']))
			  			  {
			  			      ...
			  			      $sql = "UPDATE `usertb` SET `password` = '".$_POST["password"]."', `fullName` = '".$_POST["txtFullName"]."', `address` = '".$_POST["txtAddress"]."', `gender` = '".$_POST["txtGender"]."', `job` = '".$_POST["txtJob"]."' WHERE `usertb`.`email` = '".$_SESSION["email"]."' ;";
			  			      
			  			      
			  			      
			  			      if( mysqli_query($con,$sql))
			  			      {
			  			          echo "Your Details Update successfully :) ";
			  			      }
			  			      else
    ...
    <tr>
      <td>&nbsp;</td>
      <td>  <input name="btnReset" type="reset" class="button" id="btnReset" value="Cancel" onclick="window.location.href='/news/src/main/webapp/WEB-INF/pages/home.php'"/></td>
